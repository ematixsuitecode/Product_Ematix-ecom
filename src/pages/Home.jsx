...
const QUICK_LINKS = [
  { id: 1, name: "Top Offers", img: "https://images.unsplash.com/photo-1607083206869-4c7672e72a8a?auto=format&fit=crop&w=150&q=80", color: "bg-indigo-50 border-indigo-100" },
  { id: 2, name: "Mobiles", img: "https://images.unsplash.com/photo-1598327105666-5b89351aff70?auto=format&fit=crop&w=150&q=80", color: "bg-blue-50 border-blue-100" },
  { id: 3, name: "Fashion", img: "https://images.unsplash.com/photo-1483985988355-763728e1935b?auto=format&fit=crop&w=150&q=80", color: "bg-pink-50 border-pink-100" },
  { id: 4, name: "Beauty", img: "https://images.unsplash.com/photo-1616683693504-3ea7e9ad6fec?auto=format&fit=crop&w=150&q=80", color: "bg-rose-50 border-rose-100" },
  { id: 5, name: "Home", img: "https://images.unsplash.com/photo-1583847268964-b28dc8f51f92?auto=format&fit=crop&w=150&q=80", color: "bg-orange-50 border-orange-100" },
  { id: 6, name: "Gadgets", img: "https://images.unsplash.com/photo-1498049860654-af1a5c5668ba?auto=format&fit=crop&w=150&q=80", color: "bg-gray-50 border-gray-100" },
];
...
const SLIDES = [
  {
    ...
    description: "Experience the finest collection of international brands. Free shipping on all orders above $99.",
    buttonText: "Explore Collection",
    accent: "text-yellow-400"
  },
  {
    id: 2,
    bg: "bg-gradient-to-r from-rose-100 to-rose-200", // Pink BG similar to your image
    image: "https://images.unsplash.com/photo-1596462502278-27bfdd403cc2?auto=format&fit=crop&w=1200&q=80",
    subtitle: "The Beauty Sale",
    title: "GLOW UP KITS",
    description: "Up to 50% off on top skincare brands. Presented by Maybelline & L'Oreal.",
  ...
  },
  {
    ...
    description: "Pre-order the latest consoles and accessories. Exclusive deals for Prime members.",
...
const PRODUCTS = [
  { id: 101, title: "Sony WH-1000XM5 Noise Canceling", price: 349, rating: 4.8, reviews: 2100, tag: "Best Seller", img: "https://images.unsplash.com/photo-1618366712010-f4ae9c647dcb?auto=format&fit=crop&w=600&q=80" },
  { id: 102, title: "Nike Air Jordan 1 High OG", price: 180, rating: 4.9, reviews: 850, tag: "Trending", img: "https://images.unsplash.com/photo-1552346154-21d32810aba3?auto=format&fit=crop&w=600&q=80" },
  { id: 103, title: "Apple Watch Ultra 2", price: 799, rating: 4.9, reviews: 1200, tag: "New Arrival", img: "https://images.unsplash.com/photo-1549482517-27f185ef4505?auto=format&fit=crop&w=600&q=80" },
  { id: 104, title: "Marc Jacobs Tote Bag", price: 125, rating: 4.5, reviews: 320, tag: "Limited Deal", img: "https://images.unsplash.com/photo-1591561954557-26941169b49e?auto=format&fit=crop&w=600&q=80" },
];
      ...
      <div className="flex justify-between items-start gap-6 overflow-x-auto hide-scrollbar pb-2">
        {QUICK_LINKS.map((item) => (
          <div key={item.id} className="flex flex-col items-center gap-2 cursor-pointer group min-w-[70px]">
            <div className={`w-14 h-14 md:w-16 md:h-16 rounded-full p-0.5 border ${item.color} group-hover:border-indigo-500 transition-all duration-300 overflow-hidden shadow-sm group-hover:shadow-md`}>
              <img 
                src={item.img} 
                alt={item.name} 
    ...
    <section className="relative h-[300px] md:h-[450px] w-full overflow-hidden group">
      ...
      <div className="relative w-full h-full">
        {SLIDES.map((slide, index) => (
          <div 
            key={slide.id}
            className={`absolute inset-0 transition-opacity duration-700 ease-in-out ${index === current ? 'opacity-100 z-10' : 'opacity-0 z-0'} ${slide.bg}`}
          >
            {/* Background Image (Split Layout) */}
            <div className="absolute right-0 top-0 w-1/2 h-full hidden md:block">
               <img src={slide.image} alt={slide.title} className="w-full h-full object-cover mask-image-linear-fade" style={{maskImage: 'linear-gradient(to left, black 70%, transparent 100%)', WebkitMaskImage: 'linear-gradient(to left, black 70%, transparent 100%)'}} />
            </div>
            ...
            <div className="container mx-auto px-6 h-full flex flex-col justify-center relative z-20">
               <div className={`max-w-xl ${slide.textColor || 'text-white'} animate-fade-in-up`}>
                  <span className={`inline-block font-bold px-3 py-1 text-xs uppercase tracking-wider rounded-sm mb-4 bg-white/20 backdrop-blur-md`}>
                    {slide.subtitle}
                  </span>
...
const DealZone = () => (
  // REMOVED -mt-16 to fix overlapping. Added standard spacing.
      ...
      <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6 md:p-8 grid grid-cols-1 lg:grid-cols-4 gap-8 items-center">
        ...
        <div className="lg:col-span-1 border-b lg:border-b-0 lg:border-r border-gray-100 pb-6 lg:pb-0 pr-0 lg:pr-8">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h3 className="text-xl font-black text-gray-800 tracking-tight">DEAL OF THE DAY</h3>
          </div>
          <p className="text-gray-500 text-sm mb-4 font-medium">Offer ends in:</p>
          <div className="flex gap-3">
            {['04', '32', '15'].map((time, i) => (
              <div key={i} className="flex flex-col items-center">
                ...
                <span className="text-[10px] text-gray-400 font-bold mt-1 uppercase tracking-wider">{['Hr', 'Min', 'Sec'][i]}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="lg:col-span-3">
          <div className="flex gap-4 overflow-x-auto hide-scrollbar pb-2">
            {[1, 2, 3, 4].map((_, i) => (
              <div key={i} className="min-w-[240px] flex items-center gap-4 bg-gray-50 p-4 rounded-xl border border-transparent hover:border-indigo-200 hover:bg-white hover:shadow-lg transition-all cursor-pointer group">
                <img 
                ...
                <div className="flex flex-col">
                  ...
                  <h4 className="font-bold text-gray-900 text-sm leading-tight">Nike Air Max 270</h4>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-indigo-600 font-bold">$129</span>
                    <span className="text-gray-400 text-xs line-through">$299</span>
                  </div>
                </div>
              </div>
            ))}
  ...
  <section className="py-12">
    <div className="container mx-auto px-4">
      <div className="flex justify-between items-end mb-8">
        <div>
          <span className="text-indigo-600 font-bold uppercase tracking-wider text-xs mb-1 block">Curated For You</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-auto md:h-[600px]">
        ...
        {/* Large Vertical Card */}
        <div className="md:col-span-1 md:row-span-2 relative rounded-2xl overflow-hidden group cursor-pointer shadow-md hover:shadow-xl transition-all duration-500">
          <img 
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent flex flex-col justify-end p-8">
            ...
            <h3 className="text-4xl font-white text-white font-serif italic mb-4">Minimalist</h3>
        ...
        <div className="md:col-span-2 bg-gray-100 rounded-2xl p-8 relative overflow-hidden group shadow-sm hover:shadow-lg transition-all duration-500">
          <div className="w-full md:w-1/2 relative z-10 flex flex-col justify-center h-full">
            <span className="text-indigo-600 font-bold text-xs uppercase tracking-wide mb-2">Gaming & Tech</span>
            <h3 className="text-3xl font-black text-gray-900 mb-2 leading-tight">Next Gen <br/>Consoles</h3>
            <p className="text-gray-600 mb-6 text-sm line-clamp-2">Experience 4K gaming with the new PS5 and Xbox Series X. Available now with express delivery.</p>
        ...
        {/* Small Card 1 */}
        <div className="bg-indigo-50 rounded-2xl p-6 relative overflow-hidden group cursor-pointer hover:shadow-lg transition-all">
          <div className="relative z-10">
            <h4 className="text-2xl font-bold text-gray-900 mb-1">Sneakerhead</h4>
            <p className="text-indigo-600 text-xs font-bold uppercase">New Drops Daily</p>
          </div>
          <img 
        ...
        {/* Small Card 2 */}
        <div className="bg-rose-50 rounded-2xl p-6 relative overflow-hidden group cursor-pointer hover:shadow-lg transition-all">
          <div className="relative z-10">
            <h4 className="text-2xl font-bold text-gray-900 mb-1">Glow Up Kit</h4>
            <p className="text-rose-600 text-xs font-bold uppercase">Flat 30% Off</p>
          </div>
          <img 
            ...
            <div className="flex-1 flex flex-col">
              ...
              <div className="flex items-center justify-between mt-auto pt-3 border-t border-gray-50">
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-gray-400 line-through decoration-red-500">${Math.round(product.price * 1.2)}</span>
                </div>
...
const FeatureBar = () => (
  <section className="bg-white border-t border-gray-200 py-8">
     <div className="container mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-8">
        {[
          { icon: <Zap className="w-6 h-6" />, title: "Fast Delivery", sub: "Within 24 Hours" },
          { icon: <ShieldCheck className="w-6 h-6" />, title: "Secure Payment", sub: "100% Protected" },
          { icon: <Sparkles className="w-6 h-6" />, title: "Top Quality", sub: "Verified Brands" },
          { icon: <Clock className="w-6 h-6" />, title: "24/7 Support", sub: "Always Here" },
        ].map((feat, i) => (
           <div key={i} className="flex items-center gap-4 justify-center md:justify-start">
              <div className="bg-indigo-50 text-indigo-600 p-3 rounded-full">{feat.icon}</div>
              <div>
